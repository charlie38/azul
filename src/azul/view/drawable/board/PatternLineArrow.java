...
public class PatternLineArrow extends Drawable
{
    ...
    /**
     * A player pattern arrow graphical representation.
     * @param display root.
    ...
    {
        // Arrows are drawn/animated only when this user needs to select a pattern line.
    ...
    {
        if (! mIsAnimated || ! getPlayer(mPlayerIndex).isPatternLineRowAccessible(mIndex + 1))
        {
            // Arrows are drawn only when user needs to select a pattern line, and if this line is accessible.
            return ;
        }
        ...
        Image img = mIsFocused ? getResourcesLoader().getPatternLinesArrowFocused()
                : getResourcesLoader().getPatternLinesArrow() ;
